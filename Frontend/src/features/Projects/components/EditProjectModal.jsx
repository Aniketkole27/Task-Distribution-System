...
import { updateProjectInStore } from '@/app/projectDataSlice'
...
function EditProjectModal({ project, setOpen }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // The backend update API expects teamMembers to be an array of user emails!
      const emails = formData.teamMembers.map(m => m.email).filter(Boolean);
      ...
      if (response.data?.success) {
        // Sync with Redux store directly
        dispatch(updateProjectInStore(response.data.project));
        setOpen(false);
      } else {
      ...
      setError(err.response?.data?.message || "Something went wrong while updating project")
    } finally {
      setLoading(false)
    }
  }
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/30">
          <div className='flex items-center gap-2.5'>
            ...
            <h2 className="text-lg font-extrabold tracking-tight text-foreground">Edit Project Info</h2>
          </div>
          <button 
            type="button"
            onClick={() => setOpen(false)}
        ...
        {/* Scrollable Content */}
        <div className='flex-1 overflow-y-auto max-h-[70vh] px-6 py-6 space-y-5 scrollbar-hide'>
          {error && (
          ...
          <div className='space-y-2'>
            ...
            <TeamMemberSelect
              ...
              initialSelected={project?.teamMembers || []}
            />
          </div>
        </div>
